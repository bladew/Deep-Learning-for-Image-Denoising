...
class StackedDenoisingAutoEncoder(object):
    ...
    def __init__(self, train_reader, layer = 10, batch_sz = 1, learning_rate =1e-4, pre_train = False):
        ...
        for i in xrange(len(self.w)):
            self.h = tf.matmul(next_input, self.w[i]) + self.b[i]
            self.h_ = tf.matmul(self.h, self.w_[i]) + self.b_[i])
            next_input = self.h_
        ...
        for i in xrange(len(self.das)):
            for _ in xrange(epoch):
                for x,y in self.train.read_mat(batch_sz = 20, vector = True):
                    ...
                    for j in xrange(i):
                        da = self.das[j]
                        next_input[0] = da.sess.run([da.h_], feed_dict = {da.input_corrupt: next_input[0]})
                        next_input[1] = da.sess.run([da.h_], feed_dict = {da.input_corrupt: next_input[1]})
                    da = self.das[i]
